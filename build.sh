...
parted --script cache/fos-usb.img mklabel msdos mkpart primary fat32 1 128 set 1 boot on mkpart primary ext2 129 256
...
mkfs -t ext2 -L IMAGES /dev/mapper/${LOOPDEV}p2
# fstab: LABEL=IMAGES
...
menuentry "[DEBUG] FOG Deploy from USB" {
 echo loading kernel
 linux /boot/bzImage loglevel=7 initrd=custom-init.xz root=/dev/ram0 rw ramdisk_size=275000 keymap= boottype=usb consoleblank=0 rootfstype=ext4 console=tty0 console=ttyS0,115200n8 isdebug=yes
 echo loading ram disk
 initrd /boot/custom-init.xz
 echo booting kernel...
}
